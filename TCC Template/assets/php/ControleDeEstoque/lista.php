...
<body>
	<h1 class="título" text-align:center> Listagem de <strong>Produtos</h1>
	<div class="container">
		<table class="bordered striped centered highlight responsive-table">
            ...
            <thead class="u-align-center u-custom-font u-heading-font u-palette-1-base u-table-header u-table-header-1">
				...
				<?php
					include("Conecta.php");
				$seleciona=mysqli_query($conexao, "select * from produto order by CodProduto desc");
					while ($campo=mysqli_fetch_array($seleciona)) {?>
					<tbody class="u-align-center u-table-body u-text-body-color">
						<tr style="height: 80px;">
							<td><?=$campo["Descricao"]?></td>
							<td><?=$campo["Quantidade"]?></td>
							<td>R$<?=$campo["ValorUnitario"]?></td>
							<td>R$<?=$campo["ValorTotal"]?></td>
							<td align="center"><a href="editar.php?editaCodProduto=<?=$campo["CodProduto"]?>"><i class="fa fa-edit"></i></a></td>
							<td align="center"><a href="#"onClick="verifica(<?=$campo["CodProduto"]?>)"><i class="fa fa-trash"></i></a></td>
						</tr> 
					</tbody>
				<?php }?>
